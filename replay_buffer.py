...
class ReplayBuffer:
    ...
    def get_batch(self):
        ...
        for game_id, game_history, game_prob in self.sample_n_games(self.config.batch_size):
            ...
            # We upscale the gradient if some states in the target are past the game end.
            gradient_scale_batch.append([min(
    ...
    def update_priorities(self, priorities, index_info):
        ...
        """
        for i, (game_id, game_pos) in enumerate(index_info):
            # The element could have been removed since its selection and training due to asynchronousity
            if next(iter(self.buffer)) <= game_id:
...
class Reanalyse:
    ...
    def reanalyse_once(self, replay_buffer, shared_storage):
        # Use the last model to provide a fresher, stable n-step value (See paper appendix Reanalyze)
        self.model.set_weights(shared_storage.get_info("weights"))

        games = replay_buffer.sample_n_games(1, force_uniform=True)
        assert len(games) == 1
        game_id, game_history, _ = games[0]
